...
class Players {
  ...
  public Player getNext() throws NotEnoughPlayers {
    final Player currentPlayer = getCurrent();
    Optional<Player> expectedNextPlayer
        = playerList.stream()
        .filter(player -> player.getFieldValue() != currentPlayer.getFieldValue())
        .findFirst();
  ...
  private class CreatePlayer {
    ...
    CreatePlayer withRandomFieldValue() {
      if (playerList.size() < 1) {
        ...
        fieldValue = playerList.peekLast().getFieldValue() == FieldValue.X
            ? FieldValue.O : FieldValue.X;
      }
      return this;
    }
